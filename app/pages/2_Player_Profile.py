...
from src.db.queries import player_seasons, player_team_splits, player_weekly, season_stats_for_peer_analysis
...
conn = get_db()
player_id = fuzzy_player_select("player", conn, key="profile_player")
...
seasons_df = player_seasons(conn, player_id, preset)

if seasons_df.empty:
    st.warning("No season data for this player.")
    st.stop()

player_name = seasons_df["player_name"].iloc[0] if "player_name" in seasons_df.columns else player_id
primary_pos = normalize_fantasy_position(seasons_df["position"].iloc[-1]) or seasons_df["position"].iloc[-1]
st.subheader(f"{player_name} ({primary_pos})")

career = compute_career_z(seasons_df)
...
st.markdown("### Fantasy points by season")
season_fantasy_points_chart(career)

if controls["season"]:
    splits = player_team_splits(conn, player_id, controls["season"], preset)
    if len(splits) > 1:
        st.markdown(f"### Team splits ({controls['season']})")
        split_cols = ["team", "games", "fantasy_points"] + [c for c in stat_cols if c in splits.columns]
        st.dataframe(
    ...
    weekly = player_weekly(conn, player_id, controls["season"], preset)
    if not weekly.empty:
        st.markdown(f"### Weekly ({controls['season']})")
        week_cols = ["week", "team", "fantasy_points"] + [
            c for c in stat_cols if c in weekly.columns
        ]
        st.dataframe(
